...
func NewEthereumAdapter(seed []byte, derivationPath string, isDev bool) *EthereumAdapter {
	...
	adapter.validDestinations = []string{
		"0xa2b29e213ebe225b38c8ccabd066319a2439a7e7",
	}
	return adapter
}
...
func (e *EthereumAdapter) checkDestination(payload string) (bool) {
	type payloadJson struct {
		To string `json:"to"`
	}
	var pj payloadJson
	err := json.Unmarshal([]byte(payload), &pj)
	if err != nil {
		return false
	}
	for _, address := range e.validDestinations {
		if strings.ToLower(pj.To) == strings.ToLower(address) {
			return true
		}
	}
	return false
...
func (e *EthereumAdapter) CreateSignedTransaction(payload string, backendLogger log.Logger) (string, error) {
	...
	if (!e.checkDestination(payload)) {
		return "", errors.New("Destination is not in valid addresses")
	}
